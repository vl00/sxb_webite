...
namespace Microsoft.AspNetCore.Mvc
{
    ...
    object values, string protocol, string host, string fragment)
        ...
        /// <returns></returns>
        public static string ArticleShortUrlFormat(this IUrlHelper urlHelper, long no)
        {
            return urlHelper.Action("detail", "article", new { no = UrlShortIdUtil.Long2Base32(no) });
        }
        ...
        /// <param name="dateTime"></param>
        /// <returns></returns>
        public static string ArticleCoverUrlFormat(this IUrlHelper urlHelper, article_cover cover, bool useCdn = true)
        {
            if (cover is null) return "unkown";
            string host = useCdn ? "cdn" : "file";
            return $"https://{host}.sxkid.com/images/article/{cover.articleID}/{cover.photoID}.{((FileExtension)cover.ext).ToString()}";
        }

        public static string SxbStaticFile(this IUrlHelper urlHelper, string rawUrl)
        {
            var config = urlHelper.ActionContext.HttpContext.RequestServices.GetService(typeof(IConfiguration)) as IConfiguration;
            var staticFile = config.GetSection("StaticFile").Get<string>();
            return $"{staticFile}{rawUrl}";
        }
