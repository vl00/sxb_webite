...
namespace Sxb.Web.Middleware
{
    public class LoggerMiddleware
    {
        ...
        public LoggerMiddleware(RequestDelegate next,
            IConfiguration config, IHttpClientFactory clientFactory, IWebHostEnvironment env,
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            if (log.IsInfoEnabled)
            {
                ...
                if (routes != null && routes.Count() >0 )
                {
                    ...
                    var actionDesc = endpoint?.Metadata.GetMetadata<DescriptionAttribute>()?.Description ; //ActionExtension.Description(controller, action);
