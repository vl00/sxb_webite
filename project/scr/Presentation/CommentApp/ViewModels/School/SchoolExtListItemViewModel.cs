using PMS.OperationPlateform.Domain.Enums;
using ProductManagement.Framework.Foundation;
using System;
using System.Collections.Generic;
using System.Security.Policy;

namespace Sxb.Web.ViewModels.School
{
    public class SchoolExtListItemViewModel
    {
        public SchoolExtListItemViewModel()
        {
            Tags = new List<string>();
        }
        /// <summary>
        /// 学校分部id
        /// </summary>
        public Guid BranchId { get; set; }
        /// <summary>
        /// 学校id
        /// </summary>
        public Guid SchoolId { get; set; }

        /// <summary>
        /// 学校名称
        /// </summary>
        public string SchoolName { get; set; }

        /// <summary>
        /// 是否上学帮认证
        /// </summary>
        public bool Auth { get; set; }

        /// <summary>
        /// 寄宿类型
        /// </summary>
        public int LodgingType { get; set; }
        /// <summary>
        /// 寄宿类型描述
        /// </summary>
        public string LodgingReason { get; set; }

        public int Type { get; set; }
        public string TypeName => ((SchoolType)Type).GetDescription();

        /// <summary>
        /// 是否国际学校
        /// </summary>
        public bool International { get; set; }

        /// <summary>
        /// 费用
        /// </summary>
        public double? Cost { get; set; }
        public string PerYearFee => Cost.ToPerYearFee();

        /// <summary>
        /// 距离
        /// </summary>
        public double? Distance { get; set; }
        public string DistanceString => Distance.ToDistance();

        /// <summary>
        /// 城市名称
        /// </summary>
        public string CityName { get; set; }

        /// <summary>
        /// 城区名称
        /// </summary>
        public string AreaName { get; set; }


        public List<string> Tags { get; set; }

        /// <summary>
        /// 口碑评级分
        /// </summary>
        public double? Score { get; set; }
        /// <summary>
        /// 评级
        /// </summary>
        public string Str_Score
        {
            get
            {
                if (Score.HasValue) return Score.Value.GetScoreString();
                return "";
            }
        }
        /// <summary>
        /// 星星
        /// </summary>
        public int Stars { get; set; }
        /// <summary>
        /// 学校点评总条数
        /// </summary>
        public int CommentTotal { get; set; }

        /// <summary>
        /// 学校代号
        /// </summary>
        public int SchoolNo { get; set; }
        public string ShortSchoolNo
        {
            get
            {
                return UrlShortIdUtil.Long2Base32(SchoolNo).ToLower();
            }
        }

        /// <summary>
        /// 排行榜
        /// </summary>
        public List<SchoolRankInfoViewModel> Ranks;

        /// <summary>
        /// 评论的序号
        /// </summary>
        public string CommontNo { get; set; }
        /// <summary>
        /// 评论内容
        /// </summary>
        public string Comment { get; set; }



        public class SchoolRankInfoViewModel
        {
            public Guid RankId { get; set; }

            /// <summary>
            /// 学部ID
            /// </summary>
            public Guid SchoolId { get; set; }

            /// <summary>
            /// 榜单标题名称
            /// </summary>
            public string Title { get; set; }


            /// <summary>
            /// 榜单详情链接
            /// </summary>
            public string RankUrl { get; set; }

            /// <summary>
            /// 排行榜中学部的名词
            /// </summary>
            public int Ranking { get; set; }

            public string Cover { get; set; }

            public int No { get; set; }

            public string Base32No
            {
                get
                {
                    return UrlShortIdUtil.Long2Base32(this.No);
                }
            }
        }
    }
}
