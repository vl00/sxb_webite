...
namespace Sxb.Web.Areas.Common.Models
{
    ...
    public class SearchCommentVM
    {
        ...
        public CommentSchoolVM School { get; set; }

        internal static SearchCommentVM Convert(UserCommentDto s)
        {
            var vm = new SearchCommentVM()
            {
                Id = s.Id,
                ...
                Stars = s.Stars,
                Images = s.Images,
                School = new CommentSchoolVM()
            };

            if (s.IsAnony || s.User == null)
            {
                vm.UserId = Guid.Empty;
                vm.NickName = "匿名用户";
                vm.HeadImgUrl = "".ToHeadImgUrl();
                vm.TalentType = null;
            }
