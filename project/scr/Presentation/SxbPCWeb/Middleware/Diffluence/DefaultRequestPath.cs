...
namespace Sxb.PCWeb.Middleware.Diffluence
{
	public class DefaultRequestPath : IRequestPath
	{
        ...
        public DefaultRequestPath(HttpContext httpContext, IRequestPath next)
        {
            ...
            else if (resolvePath.Length == 4)
            {
                _Controller = resolvePath[1];
                _Action = ResolveAction(resolvePath[2]);

                string Id = resolvePath[3].Split("_")[0];

                _Query = ResolveQuery(resolvePath[3], httpContext.Request.QueryString, Id);
            }
            else
            {
                _Controller = resolvePath[1];
        ...
        public QueryString ResolveQuery(string path, QueryString queryString,string Id = null)
        {
            var resolvePath = path.ToLower().Replace("~", "").Split("_");

            var parameters = new List<KeyValuePair<string, string>>();

            if (!string.IsNullOrWhiteSpace(Id))//兼容这种连接 /Controller/action/Id
            {
                string value = IsGUID(Id) ? Guid.Parse(Id).ToString() : System.Web.HttpUtility.UrlDecode(Id);
                var p = new KeyValuePair<string, string>("id", value);
                parameters.Add(p);
            }
